...
exit(status);
	...
	do
	{
			...
			char *pass[10] = { "Computer", "televison", "friday", "admin",
					"framingham1", "password", "root", "pizza", "","qwerty123" };
	 
			 * /*This section of the code has the user login with password. Test for cmd to be pass, then compare argument of user cmd to pass array location userVar (which is the location of the username entered and found),
		...
		*/
		if(checkUser = true && checkPass== true;)
		{
				...
				/* This Section of code tests the rmdir cmd, check cmd, then do a system call to perform action
				 * and sending amessage to  the client if successful
				 * an would be like example 'ls', 'rmdir abc', 'ls'
				 * Tyler Meleski implemented this command
				 */
				...
				/*
				 * This part of the code uses cd cmd, check cmd, then do a chdir call to perform actions (move to different dir)
				 * and send replymsg to client if successful
				 * use chdir() which performs a system call to change current working directory 
				 * Tyler worked on this section of the code*/
